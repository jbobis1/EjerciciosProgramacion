...
public class ControladorProfesor extends SupertipoGestion{
	...
	public static  Profesor findProfesor(String sql) {
		...
		try {
			...
			if (rs.next()) { 
				f= new Profesor (rs.getInt("id"), rs.getString("nombre"), rs.getString("apellido1"), rs.getString("apellido2"), rs.getString("Dni"), rs.getString("Direccion"), rs.getString("email"),rs.getString("telefono"));			
				}
